...
<?php
// Force redirect active and configure timeout to 2 seconds
$PORTAL_redirecturl = 'enabled'; 
$PORTAL_redirectsecs = 2;

if ( $loginstate==2 && $PORTAL_redirecturl!='X' ) {
    // Strip out any port number (like :446 or :81) to fallback to standard HTTPS on the same host/IP
    $clean_host = preg_replace('/:\d+/', '', $_SERVER['HTTP_HOST']);
    $PORTAL_redirecturl = "https://" . $clean_host . "/index.html";
    $PORTAL_redirectlogin = 0;
    
    $redirect_secs = $PORTAL_redirectsecs;
?>
